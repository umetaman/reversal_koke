...
public class SimpleMeshExploder : MonoBehaviour
{
    ...
    public int PieceNum;
    public float PieceSize,PieseScatter,mass;
    ...
    public void Explode()
    {
        ...
        GameObject Null = new GameObject("PieceParent");
        // fx
        //爆発の際の煙とかのエフェクト
...
//        Destroy(clone.gameObject,5);
        ...
        for (int i = 0; i < triangles.Length; i += 3)
        {
            ...
            CreateMeshPiece(extrudeSize, target.transform.position + vertices[triangles[i]], target.GetComponent<Renderer>().material, index, averageNormal, vertices[triangles[i]], vertices[triangles[i + 1]], vertices[triangles[i + 2]], uvs[triangles[i]], uvs[triangles[i + 1]], uvs[triangles[i + 2]],Null);
            index++;
        }
        // destroy original
    ...
    void CreateMeshPiece(float extrudeSize, Vector3 pos, Material mat, int index, Vector3 faceNormal, Vector3 v1, Vector3 v2, Vector3 v3, Vector2 uv1, Vector2 uv2, Vector2 uv3,GameObject _Null)
    {
        for (int i = 0; i < PieceNum; i++)
        {
            GameObject go = new GameObject();
            go.name = "piece_" + index;
            go.transform.parent = _Null.transform;
          ...
          Vector3[] vertices = new Vector3[3 * 4];
            ...
            RB.mass = mass+ Random.Range(0f, 2f); ;
            RB.drag = drag;
            float rnd_posX = Random.Range(-PieseScatter+ pos.x, PieseScatter+ pos.x);
            float rnd_posY = Random.Range(-PieseScatter + pos.y, PieseScatter + pos.y);
            float rnd_posZ = Random.Range(-PieseScatter + pos.z, PieseScatter + pos.z);

            go.transform.position += new Vector3(rnd_posX,rnd_posY,rnd_posZ);
            ...
            float rnd_size=Random.Range(PieceSize*0.1f, PieceSize*1f);
            go.transform.localScale = new Vector3(rnd_size, rnd_size, rnd_size);
        }
    }
